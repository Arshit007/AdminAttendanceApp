...
public class ShowAllCourseActivity extends AppCompatActivity {
    ...
    String selected_user_id,deptName,deptField,Semester;
    ...
    private void intialise() {
            ...
            uRecyclerView = findViewById(R.id.all_course_field);
    ...
    private void toolbar() {
        ...
        toolbar_title.setText(deptSpecName+" " +"Courses");
    ...
    private void showDeptField() {
        ...
        database = FirebaseDatabase.getInstance().getReference().child(deptName).child(deptField).child(deptSpecName).child(Semester);
        database.keepSynced(true);

        FirebaseRecyclerOptions<Category> options = new FirebaseRecyclerOptions.Builder<Category>()
                .setQuery(database, Category.class).build();

        adapter = new FirebaseRecyclerAdapter<Category,ShowAllCourseActivity.UserViewHolder>(options) {


            @Override
            protected void onBindViewHolder(@NonNull final ShowAllCourseActivity.UserViewHolder holder, int position, @NonNull final Category category) {
                ...
                database.child(selected_user_id).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        String CourseCredit = dataSnapshot.child("CourseCredit").getValue().toString();
                                 String CourseName = dataSnapshot.child("CourseName").getValue().toString();


                        holder.uNameValue.setText(CourseName);
                        holder.uName.setText("Course Name : ");



                        holder.uPresent.setText("Course Credit : ");
                        holder.uPresentValue.setText(CourseCredit);

                        holder.uDeptField.setText("Specialisation : ");
                        holder.uDeptFieldValue.setText(deptSpecName);
                        ...
                        holder.uTotal.setText("Course Code");
                        holder.uTotalValue.setText(dataSnapshot.getKey());



                        holder.uDept.setText("Semester : ");
                        holder.uDeptValue.setText(Semester);
            ...
            @NonNull
            @Override
            public ShowAllCourseActivity.UserViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int i) {

                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_course_detail, parent, false);

                return new ShowAllCourseActivity.UserViewHolder(view);
