...
public class MondayFragment extends Fragment {
    ...
    private void showTue() {
        ...
        final DatabaseReference   database1 = FirebaseDatabase.getInstance().getReference("Time Table").child(deptName).child(deptField).child(deptSpecName).child(Semester).child("Monday");
        database1.keepSynced(true);

        FirebaseRecyclerOptions<Category> options = new FirebaseRecyclerOptions.Builder<Category>()
                .setQuery(database1, Category.class).build();

        FirebaseRecyclerAdapter       adapter1 = new FirebaseRecyclerAdapter<Category,MondayFragment.UserViewHolder>(options) {


            @Override
            protected void onBindViewHolder(@NonNull final MondayFragment.UserViewHolder holder, int position, @NonNull final Category category) {
