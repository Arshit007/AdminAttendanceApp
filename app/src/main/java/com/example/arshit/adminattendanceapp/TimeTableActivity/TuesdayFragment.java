...
public class TuesdayFragment extends Fragment {
    ...
    private void showTue() {
        ...
        final DatabaseReference   database1 = FirebaseDatabase.getInstance().getReference("Time Table").child(deptName).child(deptField).child(deptSpecName).child(Semester).child("Tuesday");
        database1.keepSynced(true);

        FirebaseRecyclerOptions<Category> options = new FirebaseRecyclerOptions.Builder<Category>()
                .setQuery(database1, Category.class).build();

        FirebaseRecyclerAdapter       adapter1 = new FirebaseRecyclerAdapter<Category,TuesdayFragment.UserViewHolder>(options) {


            @Override
            protected void onBindViewHolder(@NonNull final TuesdayFragment.UserViewHolder holder, int position, @NonNull final Category category) {
